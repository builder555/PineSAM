...
def patched_main():
    with (
        patch("main_server.pinecil_monitor", new=AsyncMock()) as mock_monitor,
...
async def test_pinecil_monitor_broadcasts_data_from_pinecil_to_clients(mock_ws_send):
    ...
    expected_command = json.dumps(
        {"command": "LIVE_DATA", "payload": mock_live_data, "status": "OK"}
    )
    fake_pinecil = MagicMock(
        get_live_data=AsyncMock(return_value=mock_live_data), is_connected=True
    )
    with patch("main_server.pinecils", [fake_pinecil]):
        stop_event = asyncio.Event()
        monitor_task = asyncio.create_task(pinecil_monitor(stop_event))
        await complete_task(monitor_task, stop_event)

        assert mock_ws_send.call_count > 0 and mock_ws_send.call_count == len(
            mock_clients
        )
...
async def test_pinecil_monitor_reads_data_only_once_per_cycle():
    fake_pinecil = MagicMock(
        get_live_data=AsyncMock(return_value={}), is_connected=True
    )
    with patch("main_server.pinecils", [fake_pinecil]):
        stop_event = asyncio.Event()
        monitor_task = asyncio.create_task(pinecil_monitor(stop_event))
